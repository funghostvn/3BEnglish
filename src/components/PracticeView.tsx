...
export default function PracticeView({
  ...
  const fetchUserAttempts = async () => {
    ...
    try {
      ...
      const list = qs.docs.map(doc => ({ id: doc.id, ...doc.data() } as Attempt));
      const myAttempts = list.filter(a => a.userId === currentUser.id);
  ...
  const classificationFilteredExams = exams.filter(e => selectedClassificationFilter === 'all' || e.classification === selectedClassificationFilter);
  const filteredExams = classificationFilteredExams.filter(e => {
      ...
      <div className="bg-white rounded-3xl border border-slate-200/60 shadow-xs p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 border-b pb-4">
            ...
            <div className="flex flex-wrap gap-1 bg-slate-100 p-1 rounded-xl">
              ...
              <button
                type="button"
                onClick={() => setSelectedClassificationFilter('Đề thi chính thức các năm')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedClassificationFilter === 'Đề thi chính thức các năm'
                    ? 'bg-white text-indigo-600 shadow-3xs'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => setSelectedClassificationFilter('Đề thi thử từ các đơn vị')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedClassificationFilter === 'Đề thi thử từ các đơn vị'
                    ? 'bg-white text-indigo-600 shadow-3xs'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => setSelectedClassificationFilter('Đề minh họa theo chủ đề')}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedClassificationFilter === 'Đề minh họa theo chủ đề'
                    ? 'bg-white text-indigo-600 shadow-3xs'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredExams.map(exam => {
                ...
                return (
                  <div
                    key={exam.id}
                    className="p-5 border border-slate-200/60 rounded-2xl hover:border-indigo-500 hover:shadow-xs transition-all flex flex-col justify-between"
                  >
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-2">
                        <span className="bg-indigo-50 text-indigo-700 text-[10px] font-bold px-2.5 py-0.5 rounded-md uppercase">
                          Khóa: Lớp {exam.grade}
                        </span>
                        <span className="bg-slate-50 text-slate-600 text-[10px] px-2.5 py-0.5 rounded-md font-semibold font-mono">
                          Năm: {exam.year}
                        </span>
                        {exam.classification && (

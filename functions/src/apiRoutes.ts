...
// them from being committed to source control. Note: when this runs as a
// deployed Cloud Function, the gitignored JSON file never exists on disk (it
// isn't part of the deployed bundle), so it always falls through to the
// FIREBASE_* secrets there — that's expected, not a bug.
...
// Identifies this server's outbound HTTP calls (Gemini + GitHub API). Kept
// generic on purpose — this code runs both inside AI Studio and as a
// self-hosted Cloud Function, so it shouldn't claim to be "aistudio-build".
const OUTBOUND_USER_AGENT = "3benglish-app";
...
// Registers every /api/config, /api/gemini/*, and /api/github/* route onto the
// given Express app. Shared verbatim between local dev (server.ts, wrapping
// Vite) and the deployed Cloud Function (functions/src/index.ts) so the two
// never drift apart.
...
`;
  ...
  app.post("/api/github/backup", async (req, res) => {
    try {
      ...
      const backupPath = "exam_prep_backup.json";
      const url = `https://api.github.com/repos/${repo}/contents/${backupPath}`;
